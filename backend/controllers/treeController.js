...
export const getAll = async (req, res) => {
  try {
    const trees = await treeService.getAllTrees(parsePagination(req.query));
    res.json(trees);
  } catch (err) {
    console.error(err);
...
export const getMine = async (req, res) => {
  try {
    ...
    if (filter) {
      trees = await treeService.getTreesByFilter(
        req.user.id,
        filter,
        pagination,
      );
    } else {
      trees = await treeService.getMyTrees(req.user.id, pagination);
    }
...
export const addTask = async (req, res) => {
  ...
  try {
    const task = await treeService.addTreeTask(
      treeId,
      task_type,
      req.user.id,
      notes,
...
export const updateHealth = async (req, res) => {
  const { growth_status, survival_status, height_cm, photo_url } = req.body;
  const treeId = req.params.id;

  try {
    const tree = await treeService.updateTreeHealth(
      treeId,
      {
        growth_status,
        survival_status,
